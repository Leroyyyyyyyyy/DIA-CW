...
class RunnerPolymarketPMXTDataLoader(PolymarketPMXTDataLoader):
    ...
    @staticmethod
    def _fetch_json_fallback(url: str) -> Any:
        request = Request(
            url,
            headers={
                "Accept": "application/json",
                "User-Agent": _PMXT_RUNNER_HTTP_USER_AGENT,
            },
        )
    ...
    async def _fetch_market_details(cls, condition_id: str, http_client) -> dict[str, Any]:
        try:
            ...
            if not isinstance(data, dict):
                raise RuntimeError(
                    f"Unexpected market details response type: {type(data).__name__}"
                )
            return data
